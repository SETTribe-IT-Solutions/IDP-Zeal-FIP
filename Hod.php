...
// Fetch stats and recent items from DB with department filters
$total_issues = 0;
...
try {
    ...
    // Query stats filtered by Department
    ...
    FROM tbl_raiseissue WHERE department = ?");
    
    if ($count_stmt) {
        $count_stmt->bind_param("s", $user_dept);
        $count_stmt->execute();
    ...
    // Active Talukas filtered by Department
    $taluka_stmt = $conn->prepare("SELECT COUNT(DISTINCT taluka) as taluka_count FROM tbl_raiseissue WHERE department = ? AND taluka IS NOT NULL AND taluka != ''");
    if ($taluka_stmt) {
        $taluka_stmt->bind_param("s", $user_dept);
        $taluka_stmt->execute();
        $taluka_res = $taluka_stmt->get_result();
        if ($taluka_res && $row = $taluka_res->fetch_assoc()) {
            $active_talukas = (int)$row['taluka_count'];
        }
        $taluka_stmt->close();
    }

    // Recent 5 issues filtered by Department
    $recent_stmt = $conn->prepare("SELECT * FROM tbl_raiseissue WHERE department = ? ORDER BY issue_date DESC, id DESC LIMIT 5");
    if ($recent_stmt) {
        $recent_stmt->bind_param("s", $user_dept);
        $recent_stmt->execute();
...
$user_display_name = $_SESSION['user_name'] ?? "HOD Officer";
...
<main class="main-content">
    ...
    <div class="welcome-container">
        ...
        <div class="welcome-info">
            <span class="welcome-badge">
                <i class="fa-solid fa-circle-check"></i> विभाग प्रमुख डॅशबोर्ड | HOD Dashboard
            </span>
            <h1 class="welcome-title">
                <span class="highlight"><?php echo htmlspecialchars($user_display_name); ?></span>
            </h1>
            <p class="welcome-desc">
                आपण <strong><?php echo htmlspecialchars($user_dept); ?></strong> चे विभाग प्रमुख (HOD) म्हणून लॉग इन आहात. खाली आपल्या विभागांतर्गत प्रलंबित कामे, सक्रिय तालुके आणि समस्यांचा अहवाल तपासा.
            </p>
        </div>
    ...
    <div class="stats-header">
        <h2><i class="fa-solid fa-chart-pie"></i> <?php echo htmlspecialchars($user_dept); ?> आकडेवारी (Department-Specific Live Stats)</h2>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card stat-resolved">
            ...
            <div class="stat-info">
                <h3>सक्रिय तालुके</h3>
                <p class="stat-number"><?php echo $active_talukas; ?></p>
            </div>
        </div>
    </div>
    ...
    <div class="section-container">
        <div class="recent-header">
            <h2 class="section-title"><i class="fa-solid fa-list-check"></i> विभागामधील अलीकडील समस्या (Recent Department Issues)</h2>
        ...
        <div class="issues-table-card">
            ...
            <?php else: ?>
                <div class="empty-feed">
                    ...
                    <p>सध्या या विभागामध्ये कोणतीही समस्या नोंदवलेली नाही.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>

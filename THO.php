...
$user_dept = $_SESSION['user_dept'] ?? 'आरोग्य विभाग';

// Fetch stats and recent items from DB with block and dept filters
$total_issues = 0;
...
try {
    ...
    // Query stats filtered by Taluka and Department
    ...
    FROM tbl_raiseissue WHERE taluka = ? AND department = ?");
    
    if ($count_stmt) {
        $count_stmt->bind_param("ss", $user_taluka, $user_dept);
        $count_stmt->execute();
    ...
    // Active villages filtered by Taluka and Department
    $village_stmt = $conn->prepare("SELECT COUNT(DISTINCT village) as village_count FROM tbl_raiseissue WHERE taluka = ? AND department = ? AND village IS NOT NULL AND village != ''");
    if ($village_stmt) {
        $village_stmt->bind_param("ss", $user_taluka, $user_dept);
        $village_stmt->execute();
        $village_res = $village_stmt->get_result();
        if ($village_res && $row = $village_res->fetch_assoc()) {
            $active_villages = (int)$row['village_count'];
        }
        $village_stmt->close();
    }

    // Recent 5 issues filtered by Taluka and Department
    $recent_stmt = $conn->prepare("SELECT * FROM tbl_raiseissue WHERE taluka = ? AND department = ? ORDER BY issue_date DESC, id DESC LIMIT 5");
    if ($recent_stmt) {
        $recent_stmt->bind_param("ss", $user_taluka, $user_dept);
        $recent_stmt->execute();
...
$user_display_name = $_SESSION['user_name'] ?? "THO Officer";
...
<main class="main-content">
    ...
    <div class="welcome-container">
        ...
        <div class="welcome-info">
            <span class="welcome-badge">
                <i class="fa-solid fa-circle-check"></i> तालुका आरोग्य अधिकारी डॅशबोर्ड | THO Dashboard
            </span>
            <h1 class="welcome-title">
                <span class="highlight"><?php echo htmlspecialchars($user_display_name); ?></span>
            </h1>
            <p class="welcome-desc">
                आपण <strong><?php echo htmlspecialchars($user_taluka); ?></strong> तालुक्याचे तालुका आरोग्य अधिकारी (THO) म्हणून लॉग इन आहात. खाली आपल्या आरोग्य विभागाशी संबंधित प्रलंबित आणि निराकरण झालेल्या समस्या तपासा.
            </p>
        </div>
    ...
    <div class="stats-header">
        <h2><i class="fa-solid fa-chart-pie"></i> आरोग्य विभाग आकडेवारी (Health Department Stats for <?php echo htmlspecialchars($user_taluka); ?>)</h2>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card stat-resolved">
            ...
            <div class="stat-info">
                <h3>सक्रिय गावे</h3>
                <p class="stat-number"><?php echo $active_villages; ?></p>
            </div>
        </div>
    </div>
    ...
    <div class="section-container">
        <div class="recent-header">
            <h2 class="section-title"><i class="fa-solid fa-list-check"></i> आरोग्य विभागामधील अलीकडील समस्या (Recent Health Issues in <?php echo htmlspecialchars($user_taluka); ?>)</h2>
        ...
        <div class="issues-table-card">
            ...
            <?php else: ?>
                <div class="empty-feed">
                    ...
                    <p>सध्या आरोग्य विभागामध्ये कोणतीही समस्या नोंदवलेली नाही.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>

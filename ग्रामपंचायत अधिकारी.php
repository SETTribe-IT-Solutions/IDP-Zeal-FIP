...
$allowed_role = 'ग्रामपंचायत अधिकारी';
$user_system_role = $_SESSION['system_role'] ?? 'user';
if (trim($user_system_role) !== $allowed_role) {
...
// Fetch stats and recent items from DB with village filters
$total_issues = 0;
...
try {
    ...
    // Query stats filtered by Taluka and Village
    ...
    FROM tbl_raiseissue WHERE taluka = ? AND village = ?");
    
    if ($count_stmt) {
        $count_stmt->bind_param("ss", $user_taluka, $user_village);
        $count_stmt->execute();
    ...
    // Active Departments in their village
    $dept_stmt = $conn->prepare("SELECT COUNT(DISTINCT department) as dept_count FROM tbl_raiseissue WHERE taluka = ? AND village = ? AND department IS NOT NULL AND department != ''");
    if ($dept_stmt) {
        $dept_stmt->bind_param("ss", $user_taluka, $user_village);
        $dept_stmt->execute();
    ...
    // Recent 5 issues filtered by Taluka and Village
    $recent_stmt = $conn->prepare("SELECT * FROM tbl_raiseissue WHERE taluka = ? AND village = ? ORDER BY issue_date DESC, id DESC LIMIT 5");
    if ($recent_stmt) {
        $recent_stmt->bind_param("ss", $user_taluka, $user_village);
        $recent_stmt->execute();
...
$user_display_name = $_SESSION['user_name'] ?? "ग्रामपंचायत अधिकारी";
...
<main class="main-content">
    ...
    <div class="welcome-container">
        ...
        <div class="welcome-info">
            <span class="welcome-badge">
                <i class="fa-solid fa-circle-check"></i> ग्रामपंचायत अधिकारी डॅशबोर्ड | GP Officer Dashboard
            </span>
            <h1 class="welcome-title">
                <span class="highlight"><?php echo htmlspecialchars($user_display_name); ?></span>
            </h1>
            <p class="welcome-desc">
                आपण <strong><?php echo htmlspecialchars($user_village); ?> (<?php echo htmlspecialchars($user_taluka); ?>)</strong> चे ग्रामपंचायत अधिकारी म्हणून लॉग इन आहात. खाली आपल्या गावातील समस्या आणि प्रलंबित कामे पहा.
            </p>
        </div>
    ...
    <div class="stats-header">
        <h2><i class="fa-solid fa-chart-pie"></i> गावातील एकूण आकडेवारी (Grampanchayat Live Stats for <?php echo htmlspecialchars($user_village); ?>)</h2>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card stat-resolved">
            ...
            <div class="stat-info">
                <h3>कार्यरत विभाग</h3>
                <p class="stat-number"><?php echo $active_depts; ?></p>
            </div>
        </div>
    </div>
    ...
    <div class="section-container">
        <div class="recent-header">
            <h2 class="section-title"><i class="fa-solid fa-list-check"></i> गावामधील अलीकडील समस्या (Recent Issues in <?php echo htmlspecialchars($user_village); ?>)</h2>
        ...
        <div class="issues-table-card">
            ...
            <?php else: ?>
                <div class="empty-feed">
                    ...
                    <p>सध्या या गावामध्ये कोणतीही समस्या नोंदवलेली नाही.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
